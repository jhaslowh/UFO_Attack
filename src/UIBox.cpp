...
void UIBox::draw(GLHandler* mgl, UIAtlas* mUI, float offx, float offy){
	...
	// Draw borders 
	// TB
	mUI->drawScale2(mgl, UI_REC_TB, offx + loc_x + cornerWidth,  offy + loc_y, 
		width - (2.0f * cornerWidth), 1.0f);
	// BB
	mUI->drawScale2(mgl, UI_REC_BB, offx + loc_x + cornerWidth, offy + loc_y + height - cornerWidth, 
		width - (2.0f * cornerWidth), 1.0f);
	// LB
	mUI->drawScale2(mgl, UI_REC_LB, offx + loc_x, offy + loc_y + cornerWidth, 
		1.0f, height - (2.0f * cornerWidth));
	// RB
	mUI->drawScale2(mgl, UI_REC_RB, offx + loc_x + width - cornerWidth, offy + loc_y + cornerWidth, 
		1.0f, height - (2.0f * cornerWidth));

	// Draw corners 
	// TL
	mUI->draw(mgl, UI_REC_TL, offx + loc_x, offy + loc_y);
	// TR
	mUI->draw(mgl, UI_REC_TR, offx + loc_x + width - cornerWidth, offy + loc_y);
	// BL
	mUI->draw(mgl, UI_REC_BL, offx + loc_x, offy + loc_y + height - cornerWidth);
	// BR
	mUI->draw(mgl, UI_REC_BR, offx + loc_x + width - cornerWidth, offy + loc_y + height - cornerWidth);
}